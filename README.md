...
| `scripts/ensure-external-host-services.sh` | Best-effort **host** Ollama/embedding before external compose (not a full service manager). **`PROGENTO_AUTO_START_EXTERNAL=0`** disables it. Remote **`OLLAMA_URL`** hosts are not started—only probed/skipped. After auto-start it **polls** until healthy (default **120s** embedding, **90s** Ollama); override with **`PROGENTO_EMBEDDING_START_WAIT_SEC`** / **`PROGENTO_OLLAMA_START_WAIT_SEC`**. If embedding still fails, check **`progento-embedding-host.log`** — compose still starts. |
...
**Embedding start command:** **`PROGENTO_EMBEDDING_START_CMD`** runs with **cwd = this repo**. For **external** stacks, embedding is meant to run **natively on the host** (Metal/CUDA), not in Docker — the published embedding image is CPU-oriented. This repo includes **`embedding_service/`** (vendored from Progento) plus **`./scripts/start-embedding-host.sh`**. Install deps once: **`pip install -r embedding_service/requirements.txt`** (venv under `embedding_service/.venv` is recommended; the start script uses it if present). For **all-in-Docker** GPU/CPU, use **`./start.sh`** instead.

**Embedding logs:** If the helper **starts** embedding (nothing was listening on port 8002 yet), stdout/stderr go to **`progento-embedding-host.log`** in the **Progento_SelfHosted repo root** (`tail -f ./progento-embedding-host.log`). If embedding was **already running**, the helper does not start a second process and **that file may not exist**—logs stay wherever you launched the service. If you start embedding yourself, **access logs are off by default** (no `GET /models/...` lines); set **`EMBEDDING_ACCESS_LOG=1`** to enable them for debugging. Optional: **`UVICORN_LOG_LEVEL=warning`**, **`EMBEDDING_LOG_LEVEL=WARNING`** (see **`.env.example`**). Per-batch embedding work is logged at **DEBUG** only (not INFO).
