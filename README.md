...
**Embedding start command:** **`PROGENTO_EMBEDDING_START_CMD`** runs with **cwd = this repo**. For **external** stacks, embedding is meant to run **natively on the host** (Metal/CUDA), not in Docker — the published embedding image is CPU-oriented. This repo includes **`embedding_service/`** (vendored from Progento) plus **`./scripts/start-embedding-host.sh`**, which **creates `embedding_service/.venv` and runs `pip install -r requirements.txt` when needed**, then starts the server (no manual venv step required). For **all-in-Docker** GPU/CPU, use **`./start.sh`** instead.
